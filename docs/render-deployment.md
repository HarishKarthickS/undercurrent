...
3. The build runs `npm ci --include=dev` so Vite and other build tooling install even though `NODE_ENV=production`.
4. At the secret prompts, provide:
...
5. Confirm `PUBLIC_APP_URL` matches the service URL shown in Render (`https://undercurrent-demo.onrender.com` unless renamed or suffixed).
6. Wait for the API health check at `/ready` to pass. The React app and Fastify API are served from the same URL. The Render build removes the development-only `/api` gateway prefix, so the browser calls the API directly on that origin.

Create the parent account in the live app with **Sign up** (demo seeding is disabled in production). Use your chosen parent email/password there; local `PARENT_TEST_EMAIL` / `PARENT_TEST_PASSWORD` only affect `npm run seed` on a non-production machine.
